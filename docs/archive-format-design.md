...
We seriously considered four candidates: an existing single-file WordPress archive format, a proposed WordPress core export format built on ZIP, a composition of well-loved Unix archive, compression, and encryption tools, and the repository formats used by modern deduplicating backup tools. Each had real merits.

The existing single-file WordPress archive format is the closest conceptual match — a single-file WordPress archive with an openly licensed reference implementation. Adopting it would have been the fastest path to a working product. The reasons it doesn't work as the basis for `.wpmig` are accumulated, not individually decisive:

- No format version field. That format does not announce what version it was produced by. Its tooling has changed its compression algorithm multiple times over the years without any change to the file's identifier, and the result has been that backups produced by older versions cannot be reliably extracted by newer ones — a problem users have reported repeatedly. We want the opposite property — a Pontifex 5.0 must read a 0.1.0 archive without question, because the archive announces what it is and Pontifex 5.0 implements the original reader.
- No encryption. Those files are stored in plain compressed form. For a format that operators will move between hosts, share with collaborators, and store in cloud buckets, this is an unacceptable default. Adding encryption on top (e.g., by encrypting the whole file with a separate tool) loses the per-entry properties we want and produces an opaque blob that cannot be inspected without full decryption.
- No checksums or signatures. The format has no built-in way to detect corruption or tampering. A truncated file of that format is detectable only by trying to extract it; a modified file is not detectable at all.
- No manifest. To know what is in such an archive, you must read the whole archive. Random access to a single file requires scanning from the start. This is workable for migration but precludes a class of recovery operations (extract one corrupted entry to retry it; inspect the file list without unpacking).
- Lossy on metadata. That format does not preserve directory entries, file permissions, or ownership. For WordPress migration this is mostly acceptable, but for the recovery and verification use cases we want to support, it leaves us without information we need.

The proposed WordPress core export format is the most architecturally aligned candidate. It uses standard ZIP containers, has WordPress-aware directory conventions, is designed for streaming, and is being shaped by developers who care about portability. The reasons we did not adopt it:

- It is still under active design. The proposal was open and being iterated at the time we made our format decision. Adopting a moving target is risky.
...
- It carries no migration-specific metadata. The provenance fields we consider essential (source URL, source charset and collation, source plugin set, source WordPress version) are not part of that proposal. We could fit them in by convention — for example, as a JSON file at a known path — but at that point we are layering our own format on top of theirs, which is the worst of both worlds.
...
We will, however, keep our directory conventions aligned with that proposed format where they don't conflict with our other goals — anything inside a `.wpmig` archive that is meant to look like a WordPress installation should follow the same conventions, so that a future tool that can read both formats can do so without bizarre special cases.

The composition of stock Unix archive, compression, and encryption tools is technically excellent: a universally streamable archive container, a best-in-class modern compressor, and a clean modern encryption tool. The reason we didn't adopt it is the absence of a place for our metadata. We could include a `manifest.json` as the first entry in the archive, but then we have to define what that JSON looks like — which is exactly what we are doing, and at that point we have a format whose name happens to involve three other formats. Naming it after those tools would mislead users into thinking they could use stock binaries to inspect their archives, when in fact they would also need to understand our metadata layout.

The modern deduplicating backup tools were the most carefully considered candidates because their cryptography and integrity work is exemplary. They refuse to create unencrypted repositories without explicit opt-in, they have rigorous integrity checking, and they have years of production use behind them. They were not adopted because the *shape* is wrong: they are repository formats, designed for "many backups accumulating in a vault over time, deduplicating against each other." A WordPress migration is a different problem — one site, one moment, one file you can email someone or transfer over a standard protocol. Forcing repository semantics onto our use case would make routine operations (move this archive to a different host) into multi-step processes (export, then sync the repository, then import on the other side).
...
The resolution is the same one general-purpose archive formats arrived at decades ago: an index *at the end* of the file (the manifest, in our terms), with a footer pointing to it. The writer streams entries to disk without buffering; only at the end, once all entries are known, does it emit a single manifest block summarising what was written. A reader that wants random access seeks to the footer first, reads the manifest, and from it knows every entry's offset.
...
We could encrypt the entire archive as one big blob (using a whole-archive encryption tool or a streaming AES mode), or encrypt each entry independently. The first is simpler to implement and harder to get wrong cryptographically; the second is more flexible and supports partial recovery.
...
We chose passphrase-only because it matches the dominant use case (one operator, one archive, manual operation), aligns with how mainstream passphrase-based backup tools behave, and avoids the complexity of key management for users who do not need it. The trade-off is that passphrase strength becomes the operator's responsibility — a weak passphrase produces weak encryption no matter how strong the KDF and cipher.
...
The archive is streamable, but in practice writers should buffer in chunks rather than emitting bytes one at a time. A reasonable default is to flush after each entry, with internal buffering up to perhaps 1 MB per entry. For database chunks, we recommend 500 KB per chunk based on established guidance for chunked database export, which gives a sensible balance between per-chunk overhead and streaming behaviour.
...
Databases should be exported as standard SQL dump streams, split into chunks of around 500 KB each, with chunks emitted as separate entries of `kind: db_chunk`. The chunks must be reassembled in `chunk_index` order on import. This strategy follows established practice for chunked database export and is consistent with what makes large database imports tractable on PHP-time-limited hosts.
...
- **Maximum manifest size.** The manifest is JSON; even a large archive produces a manifest of a few MB. The PHP reference reader rejects manifests over 16 MiB outright — a fixed structural ceiling, not a count, so how many entries it holds depends on their shape: up to roughly 114,912 for the smallest possible entries, fewer for archives with longer paths or many database chunks. The reference reader's own configured entry-count ceiling sits below that structural maximum, at 100,000, so it never refuses a real site on count alone; the byte cap is what actually bounds it in practice. An implementation expecting larger sites may raise either figure, but must cap both at something.
...
We do this for a reason. The track record of "the only tool that can read this file is the tool that wrote it" is the track record of vendor lock-in. Every closed migration format has produced users stuck with the format because no alternative reader exists. Some existing migration formats are partially open — a reference implementation exists because a vendor published it years ago — but others are deliberately closed and proprietary. Users with archives in those closed formats are bound to their original tooling.
