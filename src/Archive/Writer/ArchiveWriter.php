<?php
/**
 * Pontifex archive writer — composes header, provenance, manifest, and footer into a complete .wpmig file.
 *
 ...
 *
 * This is the v0.1.0 commit-8 skeleton: it produces a structurally
 * valid archive with header, provenance, an empty manifest, and a
 * footer that points to the manifest. In commit 9, write_archive()
 * will be extended to accept an array of entry plans and write them
 * between the provenance and the manifest.
 *
 * On-disk layout produced by the skeleton (ARCHIVE-FORMAT.md §3):
 *
 *   [0]                     Header        (16 bytes)
 *   [16]                    Provenance    (variable; HEADER_SIZE + JSON)
 *   [...]                   (no entries yet — added in commit 9)
 ...
 * construction. Safe to call write_archive() any number of times.
 */
final class ArchiveWriter {
	 ...
	 */
	public function __construct( FooterWriter $footer_writer ) {
		$this->footer_writer = $footer_writer;
	}

	/**
	 * Write a complete empty archive (no entries) to the destination stream.
	 *
	 ...
	 */
	public function write_archive( Provenance $provenance, $destination ): int {
		if ( ! is_resource( $destination ) ) {
			throw new InvalidArgumentException( 'ArchiveWriter: $destination must be a valid stream resource.' );
		}

		// Serialise the variable-length blocks up front.
		// Provenance and ArchiveManifest can throw JsonException in principle.
		// Catch and rewrap so callers only see RuntimeException for serialisation failures.
		try {
			$provenance_bytes = $provenance->to_bytes();
			$manifest_bytes   = ( new ArchiveManifest( array() ) )->to_bytes();
			...
			throw new RuntimeException(
				...
				'ArchiveWriter: failed to serialise an archive block: ' . $e->getMessage()
			);
		}
		...
		// Manifest: remember where it starts so the footer can point at it.
