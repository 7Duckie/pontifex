...
final class FileWriter {
	 ...
	 * Kelvin sign "K" for a Latin "K", an "Å" written as one precomposed
	 * character for the same letter written as "A" plus a combining ring
	 * above — was a way past a byte-exact-then-ASCII-fold guard, on the
	 * exact filesystems this guard exists to defend. (Measured, not assumed:
	 * a plausible-looking Turkish dotless "ı" does NOT collide with a dotted
	 * "i" under this fold — it has no case-fold mapping at all, so it is not
	 * a fifth example of this and must not be reintroduced as one.)
	 *
	 ...
	 *     decomposing) leaves whole families of the same escape open. Simpler
	 *     alternatives were tried and measured, not merely reasoned about, and
	 *     each left a real gap: a Unicode-aware lower-case alone is not the
	 *     same operation as case FOLDING and misses the letters where the two
	 *     diverge (a German "ß", unchanged by a plain lower-case but folded
	 *     to "ss", chief among them); and NFKC
	 *     (compatibility) normalisation is worse than doing nothing, because
	 *     it merges pairs — the fullwidth Latin block against ordinary ASCII,
	 *     for one — that the destination filesystem itself keeps apart, which
	 *     would refuse legitimate archives no attacker had any part in.
	 *
	 ...
	 * ever reached otherwise — so unlike an earlier draft of this method,
	 * there is no silent degrade to a weaker fold to fall back to here: on a
	 * host that cannot do this properly, the restore has already been
	 * refused, honestly, rather than quietly checked with a guard that has
	 * degraded.
	 *
