<?php
/**
 * Tests for ImportCommand's pure counter helpers.
 *
...
/**
 * Tests for ImportCommand's pure counter arithmetic.
 *
 * Both merge_counters and counter_int are private static helpers with no I/O,
 * so they are exercised directly via reflection. They mirror
 * ExportCommand's, but over the import key set
 * (attempted/succeeded/failed/bytes_imported) — a separate tally, since
 * imports and exports are different facts.
 */
final class CountersTest extends TestCase {


	/**
	 * Invoke a private static method on ImportCommand via reflection.
	 *
	...
	private function invoke_static( string $method_name, ...$args ) {
		$reflection = new ReflectionMethod( ImportCommand::class, $method_name );
	...
	public function test_merge_delta_against_empty(): void {
		...
		$this->assertSame( 0, $merged['bytes_imported'] );
	}
